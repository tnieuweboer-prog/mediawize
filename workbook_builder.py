...
# -----------------------------
# Opmaak helpers (uit jouw code)
# -----------------------------
def _p(doc: Document, text: str = "", bold: bool = False, size: int = 12, align=None):
    p = doc.add_paragraph()
    run = p.add_run(text)
    run.font.name = "Arial"
...
def add_cover_page(
    ...
    *,
    ...
    # Template bepaalt header/footer/achtergrond; logo optioneel toevoegen
    if logo:
    ...
    _p(doc, "")
    _p(doc, (vak or "").upper(), bold=True, size=14)
...
# -----------------------------
# Hoofdfunctie (template + stappen)
# -----------------------------
def build_workbook_docx_front_and_steps(meta: dict, steps: List[Dict]) -> io.BytesIO:
    """
    - Laadt template op basis van vak (BWI/PIE/MVI) -> header/footer/achtergrond uit template
    - Bouwt cover page, (optioneel) materiaalstaat, stappen (ieder op eigen pagina)
    """
    template_path = _template_path_for_vak(meta.get("vak", "BWI"))
